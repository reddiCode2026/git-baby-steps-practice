...
```
98ab09e (HEAD -> main, origin/main) Update project backlog decisions and status
f3264d8 Add implementation backlog
```
...
- [x] Add a reusable search helper for `/rest/api/3/search` with JQL, pagination, requested fields, and response validation.
- [x] Implement the completed-items JQL using `statusCategory = Done AND resolved >= -7d`.
- [x] Implement the in-progress/at-risk JQL using `statusCategory = "In Progress"`.
- [x] Implement the blocker JQL using the flagged impediment or `blocked` label signals, excluding Done issues.
